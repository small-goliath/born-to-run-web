...
export default function AuthKakao() {
  ...
  const { kakaoLogin } = useAuth(authCode, { onError });
  ...
  const { handleRetry, isRetry } = useRetryHandler({
    handler: () => authCode && kakaoLogin.mutate(authCode),
  });

  return (
    <div>
      {isRetry && (kakaoLogin.isError || authCodeError) ? (
        <ErrorModal isActive={modal.isVisible} handleRetry={handleRetry} closeModal={modal.hide} backUrl="/" />
      ) : (
